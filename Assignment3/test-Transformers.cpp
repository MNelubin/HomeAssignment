...
str g1="RPG1";
str g2="RPG2";
std::unique_ptr<Association> gun_1 =std::make_unique<Association>("RPG1");
std::unique_ptr<Association> gun_2 =std::make_unique<Association>("RPG2");

Transformers First_of (Transformers::Fraction::TRANSFORMER,"BOSS","COOL",10,800,std::move(gun_1));
...
{
    ASSERT_EQ(g1,First_of.get_gun());
    First_of.set_gun(std::move(gun_2));
    ASSERT_EQ(g2,First_of.get_gun());
}
...
std::unique_ptr<Association> gun_3 =std::make_unique<Association>("RPG1");
std::unique_ptr<Association> gun_4 =std::make_unique<Association>("RPG2");

Optimus_Prime Second_of (Transformers::Fraction::TRANSFORMER,"BOSS","COOL",10,800,std::move(gun_3),11,1);
...
{
    ASSERT_EQ(g1,Second_of.get_gun());
    Second_of.set_gun(std::move(gun_4));
    ASSERT_EQ(g2,Second_of.get_gun());
}
...
std::unique_ptr<Association> gun_5 =std::make_unique<Association>("RPG1");
std::unique_ptr<Association> gun_6 =std::make_unique<Association>("RPG2");

Megatron Third_of (Transformers::Fraction::DECEPTICON,"Bad boy","Generalissimus",11,900,std::move(gun_5),8,0);
...
{
    ASSERT_EQ(g1,Third_of.get_gun());
    Third_of.set_gun(std::move(gun_6));
    ASSERT_EQ(g2,Third_of.get_gun());
}
TEST(Megatron, r_1)
{
...
std::unique_ptr<Association> gun_7 =std::make_unique<Association>("RPG1");
std::unique_ptr<Association> gun_8 =std::make_unique<Association>("RPG2");

Bamblbee Fourth_of (Transformers::Fraction::TRANSFORMER,"Bad boy","Generalissimus",11,900,std::move(gun_7),8,0);
...
{
    ASSERT_EQ(g1,Fourth_of.get_gun());
    Fourth_of.set_gun(std::move(gun_8));
    ASSERT_EQ(g2,Fourth_of.get_gun());
}
TEST(Bamblbee, r_1)
{
